...
const ProductDetail = () => {
  ...
  const getFullImageUrl = (imgPath) => {
    if (!imgPath) return 'https://via.placeholder.com/400';
  ...
  const singleImage = product.image || (product.images && product.images[0]) || '/placeholder.png';
  const rawImages =
    product.images && product.images.length >= 3
      ? product.images
      : [singleImage, singleImage, singleImage];

  const productImages = rawImages.map(getFullImageUrl);

  const relatedProducts = (products || []).filter((item) => {
    const itemId = Number(item.id || item._id);
