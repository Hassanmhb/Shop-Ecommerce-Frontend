...
const CartPage = () => {
  ...
  const { cartItems, updateQuantity, removeFromCart, API_BASE } = useContext(ProductContext);
  ...
  // Cloudinary / Relative Image Path Formatter
    ...
    <Box sx={{ backgroundColor: '#FFFFFF', minHeight: '100vh', width: '100%', overflowX: 'hidden' }}>
      ...
      <Box sx={{ maxWidth: '1240px', width: '100%', mx: 'auto', px: { xs: 2, sm: 3, md: 4 }, py: { xs: 2, md: 3 }, boxSizing: 'border-box' }}>
        ...
        >
              ...
              cartItems.map((item, index) => {
                ...
                return (
                  <React.Fragment key={`${itemId}-${index}`}>
                    <Box sx={{ display: 'flex', gap: { xs: 1.5, sm: 2 }, alignItems: 'stretch', width: '100%' }}>
                      <Box
                        sx={{
                      ...
                      <Box sx={{ flex: 1, display: 'flex', justifyContent: 'space-between', flexDirection: 'row' }}>
                        <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                          <Box>
                            <Typography variant="h6" sx={{ fontWeight: 700, fontSize: { xs: '15px', sm: '18px', md: '20px' }, color: '#000000' }}>
                          ...
                          <Typography variant="h6" sx={{ fontWeight: 700, fontSize: { xs: '18px', sm: '22px' }, color: '#000000' }}>
                            ${item.price}
                          </Typography>
                        </Box>

                        <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                          <IconButton onClick={() => removeFromCart(itemId)} sx={{ color: '#FF3333', p: 0 }}>
                            <DeleteIcon sx={{ fontSize: { xs: '20px', sm: '22px' } }} />
                          </IconButton>

                          <Box sx={{ display: 'flex', alignItems: 'center', backgroundColor: '#F0F0F0', borderRadius: '62px', px: { xs: 1, sm: 1.5 }, py: 0.3 }}>
                            <IconButton size="small" onClick={() => updateQuantity(itemId, 'decrease')} sx={{ color: '#000', p: 0.2 }}>
                              <RemoveIcon sx={{ fontSize: '16px' }} />
                            </IconButton>
                            <Typography sx={{ fontWeight: 600, px: 1.5, fontSize: '14px' }}>
                              {item.quantity}
                            </Typography>
                            <IconButton size="small" onClick={() => updateQuantity(itemId, 'increase')} sx={{ color: '#000', p: 0.2 }}>
          ...
          {/* Summary Box */}
          <Box sx={{ flex: { xs: '1 1 100%', md: '0 0 36%' }, width: '100%', border: '1px solid rgba(0, 0, 0, 0.1)', borderRadius: '20px', p: { xs: 2.5, sm: 3 }, display: 'flex', flexDirection: 'column', gap: 2, boxSizing: 'border-box' }}>
            <Typography variant="h6" sx={{ fontWeight: 700, fontSize: '20px', color: '#000' }}>Order Summary</Typography>
            ...
            {promoError && <Alert severity="error" sx={{ borderRadius: '12px', fontSize: '13px', py: 0 }}>{promoError}</Alert>}
            {promoSuccess && (
              <Alert severity="success" action={<IconButton size="small" onClick={handleRemovePromo}><CloseIcon fontSize="small" /></IconButton>} sx={{ borderRadius: '12px', fontSize: '13px', py: 0 }}>
                {promoSuccess}
              </Alert>
            )}
            ...
            <Box sx={{ display: 'flex', gap: 1.5, mt: 1 }}>
              ...
              />
              <Button variant="contained" onClick={handleApplyPromo} sx={{ backgroundColor: '#000000', borderRadius: '62px', px: 3.5, textTransform: 'none', fontWeight: 500, boxShadow: 'none', '&:hover': { backgroundColor: '#333333' } }}>
                Apply
              </Button>
            </Box>
